...
import IdealogsPlugin from "../main";
...
describe("ArticleSuggestPlugin", () => {
  let app: App;
  let plugin: IdealogsPlugin;

  beforeEach(() => {
    app = new App();
    plugin = new IdealogsPlugin(app, {
      id: "test-plugin",
      name: "Test Plugin",
      version: "1.0.0",
    } as any);
  ...
  describe("isIdealogsArticle", () => {
    ...
    it("should not recognize non-Idealogs patterns", async () => {
      ...
      expect((plugin as any).isIdealogsArticle("regular-note")).toBe(false);
  ...
  describe("openAnnotatorViewByFile", () => {
    ...
    it("should reuse existing annotator leaf if available", async () => {
      ...
      jest
        ...
        .mockReturnValue([mockLeaf as any]);
  ...
  describe("patchLinkOpening", () => {
    ...
    it("should intercept Idealogs article links", async () => {
      ...
      const result = await app.workspace.openLinkText("@Tx123", "", false);
  ...
  describe("setupReaderButton", () => {
    it("should add reader button to markdown view", async () => {
      ...
      const button = mockContainerEl.querySelector(".idealogs-reader-button");

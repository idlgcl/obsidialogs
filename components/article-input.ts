...
export class ArticleAutocompleteField extends Component {
  ...
  private createField(): void {
    ...
    const fieldContainer = container.createDiv({ cls: 'article-autocomplete-field' });
    
    if (this.options.label) {
      fieldContainer.createEl('label', { text: this.options.label });
    }
    
    const inputContainer = fieldContainer.createDiv({ cls: 'article-input-container' });
  ...
  private onDocumentClick(event: MouseEvent): void {
    ...
    const isInsideComponent = target.closest('.article-autocomplete-field') || 
  ...
  private renderSuggestions(): void {
    ...
    this.suggestions.forEach(article => {
      const suggestionEl = this.suggestionsEl.createDiv({ cls: 'article-suggestion-item' });
      
      const container = suggestionEl.createDiv({ cls: 'idealogs-suggestion-container' });

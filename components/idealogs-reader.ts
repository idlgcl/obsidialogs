...
export class IdealogsReaderView extends ItemView {
  ...
  private annotationsByWordIndex: Map<
    number,
    { annotation: AnnotationData; type: "comment" | "note" }[]
  > = new Map();
  ...
  constructor(leaf: WorkspaceLeaf) {
    ...
    this.articleHeaderEl = this.contentEl.createDiv({
      cls: "idealogs-article-header",
    });
    this.articleContentEl = this.contentEl.createDiv({
      cls: "idealogs-article-content",
    });
    ...
    switch (kind.toLowerCase()) {
      ...
      case "writing": {
        if (!IdealogsReaderView.writingNumbers.has(articleId)) {
          IdealogsReaderView.writingNumbers.set(
            articleId,
            IdealogsReaderView.nextWritingNumber++
          );
        }

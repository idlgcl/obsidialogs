...
export class NoteForm extends Component {
  ...
  private async handleSave(): Promise<void> {
    ...
    try {
      ...
      const targetStartIndex = parseInt(
        targetStartSpans[0].getAttribute("data-word-index") || "0"
      );
      const targetEndIndex = parseInt(
        targetEndSpans[targetEndSpans.length - 1].getAttribute(
          "data-word-index"
        ) || "0"
      );

      const targetRangeSpans = this.getSpansBetweenIndices(
        wordSpans,
        targetStartIndex,
        targetEndIndex
      );
      ...
      const targetDisplaySpans = this.findTextSpansInRange(
        targetRangeSpans,
        targetTextDisplay
      );
  ...
  private getAllWordSpansFromArticleView(
    articleView: IdealogsAnnotator
  ): HTMLElement[] {
    const contentEl = articleView.contentEl;
    if (!contentEl) return [];

    const articleContentEl = contentEl.querySelector(
      ".idealogs-article-content"
    );
  ...
  private findTextSpansInRange(
    rangeSpans: HTMLElement[],
    text: string
  ): HTMLElement[] {
    const words = text.split(/\s+/).filter((w) => w.length > 0);
    if (words.length === 0) return [];
  ...
  private getSpansBetweenIndices(
    ...
    return allSpans
      .filter((span) => {
        const indexAttr = parseInt(
          span.getAttribute("data-word-index") || "-1"
        );

...
export class FileHandler {
  ...
  async handleFileOpen(file: TFile): Promise<void> {
    ...
    if (file instanceof TFile && file.extension === "md") {
      const isIdealogsFile = IDEALOGS_FILE_PATTERNS.some((pattern) =>
        file.basename.startsWith(pattern)
      );
      ...
      if (isIdealogsFile) {
        ...
        const existingRightPanelLeaves =
          this.app.workspace.getLeavesOfType(IDL_RIGHT_PANEL);
